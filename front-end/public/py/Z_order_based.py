...
def Zorder_auto_voting(filename, m, sample_rate):
    ...
    with open("./z-order/order_healthy_output_{}.json".format(m.k), mode='w', encoding='utf8') as f:
        res = []
        for i in range(len(A)):
            neighbors = m.neighbors[i]
            res.append({
    ...
    with open('./z-order/order_healthy_output_{}.json'.format(m.k), 'r', encoding='utf-8') as f:
        for i in f:
            RecomputingPoints = json.loads(i)
    oriData = A
    correct = 0
