...
def z_replacement1(filename, m, beta, oriname, near, outputname):
    # 优化选点
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        possionCenter = json.load(f)
    samplingR = len(possionCenter)
    with open(oriname, 'r', encoding='utf-8') as f:
        oriData = json.load(f)
    with open(near, 'r', encoding='utf-8') as f:
        diskCoverPoints = json.load(f)
...
if __name__ == '__main__':
    ...
    for index, i in enumerate(t):
        ...
        if index < len(t) - 1:
            ...
            recentBlueNoiseFilePath2 += temp
        else:
            recentBlueNoiseFilePath1 += (temp1[0]+'_o.json')
            recentBlueNoiseFilePath2 += 'nearbyM1.json'
    recentBlueNoiseFilePath1 = filename.replace("_o", "_ob")
    m = Z_score(k=8, mode="euclidean", equal=False)
    for i in range(int(iteration)):
        if i == 0:
            a, b, c, d, e = z_replacement1(filename, m, beta, oriname, recentBlueNoiseFilePath2, recentBlueNoiseFilePath1)
        else:
            filename = recentBlueNoiseFilePath1
            a, b, c, d, e = z_replacement1(filename, m, beta, oriname, recentBlueNoiseFilePath2, recentBlueNoiseFilePath1)
    # print(b)
    pass
#
