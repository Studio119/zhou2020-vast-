...
def sorting(points, samplePs, activepoints, sampleBRand2R, meann):
    ...
    for index, i in enumerate(points):
        ...
        # temp_value[str(index)] = abs(i['value'] - value - licha)
...
def blueNoise(originalPoints, r, value):
    ...
    mean_value = initialActivePoint['value']
    ...
    while (len(activePoints) > 0 or ifAllPointsInDisk(points, samplePoints) == False):
        ...
        # samplePointsBetweenRand2R = [x for x in samplePoints if x in pointsBetweenRand2R]
        # samplePPS = copy.deepcopy(samplePoints)
        # pointsBetweenRand2R = sorting(pointsBetweenRand2R, samplePPS, randomActivePoint, samplePointsBetweenRand2R, mean_value)
        for p1 in pointsBetweenRand2R:
            ...
            for p2 in samplePoints:
                ...
                mean_value += ((len(samplePoints)-1)/len(samplePoints)) * mean_value + (1/(len(samplePoints)))*p1['value']
                logging.info('sampling points:{0}'.format(len(samplePoints)))
                break
        else:
...
if __name__ == '__main__':
    ...
    for r in [100000]:
        ...
        t1 = time.time()
        points = []
        # with open('./outputdata/kde_local_M1.json', 'r', encoding='utf-8') as f:
        #     for i in f:
        #         LC = json.loads(i)
        with open('./inputdata/healthy_output_101.json', 'r', encoding='utf-8') as f:
            for i in f:
                temp = json.loads(i)
            value = 0
        ...
        #     print(e)
        recentBlueNoiseFilePath = './blue noise/_bbbsamplePoints-{0}-{1}-{2}.json'.format(r, len(samplePoints),
        ...
        with open(recentBlueNoiseFilePath, 'w',
            ...
            if (len(samplePoints) > maxCount):
                ...
                maxR = r
                with open('./blue noise/samplePoints.json', 'w', encoding='utf-8') as f2:
